"""数据库模块 - 使用 SQLite 数据库存储数据"""
import aiosqlite
import json
import sqlite3
from pathlib import Path
from typing import Dict, List, Optional, Any
from datetime import datetime
from nonebot.log import logger


class Database:
    """数据库管理类"""

    def __init__(self):
        """初始化数据库 —— 仅设置路径，不执行异步初始化"""
        import nonebot_plugin_localstore as store

        self.data_path: Path = store.get_plugin_data_dir()
        self.data_path.mkdir(parents=True, exist_ok=True)
        self.db_file: Path = self.data_path / "maimai_raking.db"
        self._initialized: bool = False

    async def init(self):
        """异步初始化数据库表结构（需在事件循环启动后调用）"""
        if self._initialized:
            return
        await self._init_database()
        self._initialized = True

    async def _init_database(self):
        """初始化数据库表结构"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row

            # 创建群组表
            await db.execute("""
                CREATE TABLE IF NOT EXISTS groups (
                    group_id TEXT PRIMARY KEY,
                    enabled INTEGER NOT NULL DEFAULT 1,
                    wmrt_enabled INTEGER NOT NULL DEFAULT 1,
                    created_at TEXT NOT NULL
                )
            """)

            # 兼容旧版：检查并添加 wmrt_enabled 列（如果不存在）
            try:
                await db.execute(
                    "ALTER TABLE groups ADD COLUMN wmrt_enabled INTEGER NOT NULL DEFAULT 1"
                )
                logger.info("已为groups表添加wmrt_enabled列")
            except Exception:
                pass  # 列已存在

            # 创建用户表
            await db.execute("""
                CREATE TABLE IF NOT EXISTS users (
                    qq TEXT PRIMARY KEY,
                    joined_at TEXT NOT NULL
                )
            """)

            # 创建用户-群组关系表
            await db.execute("""
                CREATE TABLE IF NOT EXISTS user_groups (
                    qq TEXT NOT NULL,
                    group_id TEXT NOT NULL,
                    PRIMARY KEY (qq, group_id),
                    FOREIGN KEY (qq) REFERENCES users(qq),
                    FOREIGN KEY (group_id) REFERENCES groups(group_id)
                )
            """)

            # 创建成绩记录表
            await db.execute("""
                CREATE TABLE IF NOT EXISTS records (
                    qq TEXT PRIMARY KEY,
                    data TEXT NOT NULL,
                    updated_at TEXT NOT NULL,
                    FOREIGN KEY (qq) REFERENCES users(qq)
                )
            """)

            # 创建刷新记录表（用于频率限制）
            await db.execute("""
                CREATE TABLE IF NOT EXISTS refresh_logs (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    qq TEXT NOT NULL,
                    refresh_date TEXT NOT NULL,
                    refresh_time TEXT NOT NULL,
                    FOREIGN KEY (qq) REFERENCES users(qq)
                )
            """)

            # 创建自定义别名表
            await db.execute("""
                CREATE TABLE IF NOT EXISTS custom_alias (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    song_id INTEGER NOT NULL,
                    alias TEXT NOT NULL COLLATE NOCASE,
                    created_at TEXT NOT NULL,
                    updated_at TEXT NOT NULL,
                    UNIQUE(alias)
                )
            """)
            await db.execute("""
                CREATE INDEX IF NOT EXISTS idx_custom_alias_song_id
                ON custom_alias(song_id)
            """)

            # 创建索引
            await db.execute("""
                CREATE INDEX IF NOT EXISTS idx_user_groups_group_id
                ON user_groups(group_id)
            """)
            await db.execute("""
                CREATE INDEX IF NOT EXISTS idx_user_groups_qq
                ON user_groups(qq)
            """)

            await db.commit()
            logger.info("数据库初始化完成")

    # ==================== 群组管理 ====================

    async def enable_group(self, group_id: str):
        """启用群组功能"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                await db.execute(
                    "INSERT OR IGNORE INTO groups (group_id, enabled, wmrt_enabled, created_at) VALUES (?, 1, 1, ?)",
                    (group_id, datetime.now().isoformat())
                )
                await db.execute(
                    "UPDATE groups SET enabled = 1 WHERE group_id = ?",
                    (group_id,)
                )
                await db.commit()
                logger.info(f"群组 {group_id} 功能已启用")
            except Exception as e:
                await db.rollback()
                logger.error(f"启用群组 {group_id} 功能失败: {e}")

    async def disable_group(self, group_id: str):
        """禁用群组功能"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                await db.execute(
                    "INSERT OR IGNORE INTO groups (group_id, enabled, wmrt_enabled, created_at) VALUES (?, 0, 1, ?)",
                    (group_id, datetime.now().isoformat())
                )
                await db.execute(
                    "UPDATE groups SET enabled = 0 WHERE group_id = ?",
                    (group_id,)
                )
                await db.commit()
                logger.info(f"群组 {group_id} 功能已禁用")
            except Exception as e:
                await db.rollback()
                logger.error(f"禁用群组 {group_id} 功能失败: {e}")

    async def enable_wmrt(self, group_id: str):
        """启用群组的wmrt功能"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                await db.execute(
                    "INSERT OR IGNORE INTO groups (group_id, enabled, wmrt_enabled, created_at) VALUES (?, 1, 1, ?)",
                    (group_id, datetime.now().isoformat())
                )
                await db.execute(
                    "UPDATE groups SET wmrt_enabled = 1 WHERE group_id = ?",
                    (group_id,)
                )
                await db.commit()
                logger.info(f"群组 {group_id} 的wmrt功能已启用")
            except Exception as e:
                await db.rollback()
                logger.error(f"启用群组 {group_id} 的wmrt功能失败: {e}")

    async def disable_wmrt(self, group_id: str):
        """禁用群组的wmrt功能"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                await db.execute(
                    "INSERT OR IGNORE INTO groups (group_id, enabled, wmrt_enabled, created_at) VALUES (?, 1, 0, ?)",
                    (group_id, datetime.now().isoformat())
                )
                await db.execute(
                    "UPDATE groups SET wmrt_enabled = 0 WHERE group_id = ?",
                    (group_id,)
                )
                await db.commit()
                logger.info(f"群组 {group_id} 的wmrt功能已禁用")
            except Exception as e:
                await db.rollback()
                logger.error(f"禁用群组 {group_id} 的wmrt功能失败: {e}")

    async def is_group_enabled(self, group_id: str) -> bool:
        """检查群组是否启用"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                cursor = await db.execute(
                    "SELECT enabled FROM groups WHERE group_id = ?",
                    (group_id,)
                )
                row = await cursor.fetchone()
                return row["enabled"] == 1 if row else False
            except Exception as e:
                logger.error(f"检查群组 {group_id} 启用状态失败: {e}")
                return False

    async def is_wmrt_enabled(self, group_id: str) -> bool:
        """检查群组的wmrt功能是否启用"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                cursor = await db.execute(
                    "SELECT wmrt_enabled FROM groups WHERE group_id = ?",
                    (group_id,)
                )
                row = await cursor.fetchone()
                return row["wmrt_enabled"] == 1 if row and row["wmrt_enabled"] is not None else True
            except Exception as e:
                logger.error(f"检查群组 {group_id} 的wmrt功能启用状态失败: {e}")
                return True

    async def get_all_enabled_groups(self) -> List[str]:
        """获取所有启用的群组"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                cursor = await db.execute("SELECT group_id FROM groups WHERE enabled = 1")
                rows = await cursor.fetchall()
                return [row["group_id"] for row in rows]
            except Exception as e:
                logger.error(f"获取所有启用群组列表失败: {e}")
                return []

    # ==================== 用户管理 ====================

    async def add_user_to_group(self, qq: str, group_id: str):
        """添加用户到群组"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                # 确保群组存在
                cursor = await db.execute(
                    "SELECT group_id FROM groups WHERE group_id = ?",
                    (group_id,)
                )
                if not await cursor.fetchone():
                    await db.execute(
                        "INSERT INTO groups (group_id, enabled, created_at) VALUES (?, 1, ?)",
                        (group_id, datetime.now().isoformat())
                    )

                # 确保用户存在
                cursor = await db.execute(
                    "SELECT qq FROM users WHERE qq = ?",
                    (qq,)
                )
                if not await cursor.fetchone():
                    await db.execute(
                        "INSERT INTO users (qq, joined_at) VALUES (?, ?)",
                        (qq, datetime.now().isoformat())
                    )

                # 添加用户-群组关系
                await db.execute(
                    "INSERT OR IGNORE INTO user_groups (qq, group_id) VALUES (?, ?)",
                    (qq, group_id)
                )

                await db.commit()
                logger.info(f"用户 {qq} 已加入群组 {group_id} 的排行榜")
            except Exception as e:
                await db.rollback()
                logger.error(f"添加用户 {qq} 到群组 {group_id} 失败: {e}")

    async def remove_user_from_group(self, qq: str, group_id: str):
        """从群组移除用户"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                await db.execute(
                    "DELETE FROM user_groups WHERE qq = ? AND group_id = ?",
                    (qq, group_id)
                )
                await db.commit()
                logger.info(f"用户 {qq} 已从群组 {group_id} 的排行榜中退出")
            except Exception as e:
                await db.rollback()
                logger.error(f"从群组 {group_id} 移除用户 {qq} 失败: {e}")

    async def is_user_in_group(self, qq: str, group_id: str) -> bool:
        """检查用户是否在群组中"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                cursor = await db.execute(
                    "SELECT 1 FROM user_groups WHERE qq = ? AND group_id = ?",
                    (qq, group_id)
                )
                return await cursor.fetchone() is not None
            except Exception as e:
                logger.error(f"检查用户 {qq} 是否在群组 {group_id} 失败: {e}")
                return False

    async def get_group_users(self, group_id: str) -> List[str]:
        """获取群组的所有用户"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                cursor = await db.execute(
                    "SELECT qq FROM user_groups WHERE group_id = ?",
                    (group_id,)
                )
                rows = await cursor.fetchall()
                return [row["qq"] for row in rows]
            except Exception as e:
                logger.error(f"获取群组 {group_id} 的用户列表失败: {e}")
                return []

    async def get_all_users(self) -> List[str]:
        """获取所有用户"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                cursor = await db.execute("SELECT qq FROM users")
                rows = await cursor.fetchall()
                return [row["qq"] for row in rows]
            except Exception as e:
                logger.error(f"获取所有用户列表失败: {e}")
                return []

    async def get_all_groups(self) -> List[str]:
        """获取数据库中所有群组"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                cursor = await db.execute("SELECT group_id FROM groups")
                rows = await cursor.fetchall()
                return [row["group_id"] for row in rows]
            except Exception as e:
                logger.error(f"获取所有群组列表失败: {e}")
                return []

    async def remove_invalid_user_from_group(self, qq: str, group_id: str):
        """从单个群移除不存在的用户，同时检查该用户是否还有其他群，如没有则清理成绩记录"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                # 首先从该群移除用户
                await db.execute(
                    "DELETE FROM user_groups WHERE qq = ? AND group_id = ?",
                    (qq, group_id)
                )

                # 检查该用户是否还属于其他群
                cursor = await db.execute(
                    "SELECT 1 FROM user_groups WHERE qq = ?",
                    (qq,)
                )
                has_other_groups = await cursor.fetchone() is not None

                # 如果没有其他群，清理成绩和用户记录
                if not has_other_groups:
                    await db.execute("DELETE FROM records WHERE qq = ?", (qq,))
                    await db.execute("DELETE FROM refresh_logs WHERE qq = ?", (qq,))
                    await db.execute("DELETE FROM users WHERE qq = ?", (qq,))
                    logger.info(f"用户 {qq} 已从所有群移除，清理成绩记录")
                else:
                    logger.info(f"用户 {qq} 已从群组 {group_id} 移除，但仍属于其他群")

                await db.commit()
            except Exception as e:
                await db.rollback()
                logger.error(f"从群组 {group_id} 移除无效用户 {qq} 失败: {e}")

    async def clean_left_groups(self, current_groups: List[str]) -> int:
        """清理已退出的群组数据

        Returns:
            清理的群组数量
        """
        cleaned_count = 0

        # 先获取所有群组列表
        all_groups = await self.get_all_groups()
        left_groups = [gid for gid in all_groups if gid not in current_groups]

        if not left_groups:
            return 0

        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                users_to_clean_records: set = set()
                remaining_users: set = set()

                # 删除已退出群组的相关数据
                for group_id in left_groups:
                    # 获取群组中的所有用户
                    cursor = await db.execute(
                        "SELECT qq FROM user_groups WHERE group_id = ?",
                        (group_id,)
                    )
                    rows = await cursor.fetchall()
                    group_users = [row["qq"] for row in rows]
                    users_to_clean_records.update(group_users)

                    # 删除群组中的用户关系
                    await db.execute(
                        "DELETE FROM user_groups WHERE group_id = ?",
                        (group_id,)
                    )
                    # 删除群组记录
                    await db.execute(
                        "DELETE FROM groups WHERE group_id = ?",
                        (group_id,)
                    )
                    cleaned_count += 1
                    logger.info(f"已清理退出的群组数据: {group_id}")

                # 获取仍然存在的群组中的用户
                for group_id in current_groups:
                    cursor = await db.execute(
                        "SELECT qq FROM user_groups WHERE group_id = ?",
                        (group_id,)
                    )
                    rows = await cursor.fetchall()
                    remaining_users.update(row["qq"] for row in rows)

                # 清理不再属于任何群组的用户的成绩记录
                users_to_delete = users_to_clean_records - remaining_users
                for user_qq in users_to_delete:
                    await db.execute("DELETE FROM records WHERE qq = ?", (user_qq,))
                    logger.info(f"已清理用户 {user_qq} 的成绩记录")

                await db.commit()
                logger.info(f"共清理了 {cleaned_count} 个已退出群组的数据")
                logger.info(f"共清理了 {len(users_to_delete)} 个用户的记录数据")
            except Exception as e:
                await db.rollback()
                logger.error(f"清理已退出群组数据失败: {e}")
                return 0

        return cleaned_count

    # ==================== 成绩管理 ====================

    async def update_user_records(self, qq: str, records: dict):
        """更新用户成绩"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                data_json = json.dumps(records, ensure_ascii=False)
                updated_at = datetime.now().isoformat()
                await db.execute(
                    "INSERT OR REPLACE INTO records (qq, data, updated_at) VALUES (?, ?, ?)",
                    (qq, data_json, updated_at)
                )
                await db.commit()
                logger.info(f"用户 {qq} 的成绩已更新")
            except Exception as e:
                await db.rollback()
                logger.error(f"更新用户 {qq} 的成绩失败: {e}")

    async def get_user_records(self, qq: str) -> Optional[dict]:
        """获取用户成绩"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                cursor = await db.execute(
                    "SELECT data FROM records WHERE qq = ?",
                    (qq,)
                )
                row = await cursor.fetchone()
                if row:
                    return json.loads(row["data"])
                return None
            except Exception as e:
                logger.error(f"获取用户 {qq} 的成绩失败: {e}")
                return None

    async def get_last_update_time(self, qq: str) -> Optional[str]:
        """获取用户成绩的最后更新时间"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                cursor = await db.execute(
                    "SELECT updated_at FROM records WHERE qq = ?",
                    (qq,)
                )
                row = await cursor.fetchone()
                return row["updated_at"] if row else None
            except Exception as e:
                logger.error(f"获取用户 {qq} 的更新时间失败: {e}")
                return None

    async def get_daily_refresh_count(self, qq: str, date: str) -> int:
        """获取用户指定日期的刷新次数"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                cursor = await db.execute(
                    "SELECT COUNT(*) as count FROM refresh_logs WHERE qq = ? AND refresh_date = ?",
                    (qq, date)
                )
                row = await cursor.fetchone()
                return row["count"] if row else 0
            except Exception as e:
                logger.error(f"获取用户 {qq} 的刷新次数失败: {e}")
                return 0

    async def log_refresh(self, qq: str, date: str):
        """记录用户刷新操作"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                refresh_time = datetime.now().isoformat()
                await db.execute(
                    "INSERT INTO refresh_logs (qq, refresh_date, refresh_time) VALUES (?, ?, ?)",
                    (qq, date, refresh_time)
                )
                await db.commit()
                logger.info(f"记录用户 {qq} 的刷新操作: {date}")
            except Exception as e:
                await db.rollback()
                logger.error(f"记录用户 {qq} 的刷新操作失败: {e}")

    async def reset_daily_refresh_count(self, qq: str, date: str):
        """重置用户指定日期的刷新次数"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                await db.execute(
                    "DELETE FROM refresh_logs WHERE qq = ? AND refresh_date = ?",
                    (qq, date)
                )
                await db.commit()
                logger.info(f"重置用户 {qq} 的刷新次数: {date}")
            except Exception as e:
                await db.rollback()
                logger.error(f"重置用户 {qq} 的刷新次数失败: {e}")

    # ==================== 自定义别名管理 ====================

    async def add_custom_alias(self, song_id: int, alias: str) -> bool:
        """新增自定义别名"""
        alias = alias.strip()
        if not alias:
            return False
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                now = datetime.now().isoformat()
                await db.execute(
                    "INSERT INTO custom_alias (song_id, alias, created_at, updated_at) VALUES (?, ?, ?, ?)",
                    (int(song_id), alias, now, now)
                )
                await db.commit()
                logger.info(f"为歌曲 {song_id} 新增自定义别名: {alias}")
                return True
            except Exception as e:
                await db.rollback()
                logger.warning(f"新增自定义别名失败: song_id={song_id}, alias={alias}, error={e}")
                return False

    async def remove_custom_alias(self, song_id: int, alias: str) -> bool:
        """移除自定义别名"""
        alias = alias.strip()
        if not alias:
            return False
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                cursor = await db.execute(
                    "DELETE FROM custom_alias WHERE song_id = ? AND alias = ?",
                    (int(song_id), alias)
                )
                removed = cursor.rowcount > 0
                if removed:
                    await db.commit()
                    logger.info(f"移除歌曲 {song_id} 的自定义别名: {alias}")
                else:
                    await db.rollback()
                return removed
            except Exception as e:
                await db.rollback()
                logger.error(f"移除自定义别名失败: song_id={song_id}, alias={alias}, error={e}")
                return False

    async def get_custom_aliases(self, song_id: int) -> List[str]:
        """获取指定歌曲的所有自定义别名"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                cursor = await db.execute(
                    "SELECT alias FROM custom_alias WHERE song_id = ? ORDER BY alias COLLATE NOCASE",
                    (int(song_id),)
                )
                rows = await cursor.fetchall()
                return [row["alias"] for row in rows]
            except Exception as e:
                logger.error(f"获取歌曲 {song_id} 的自定义别名失败: {e}")
                return []

    async def get_all_custom_aliases(self) -> Dict[int, List[str]]:
        """获取所有自定义别名"""
        async with aiosqlite.connect(self.db_file) as db:
            db.row_factory = sqlite3.Row
            try:
                cursor = await db.execute(
                    "SELECT song_id, alias FROM custom_alias ORDER BY song_id, alias COLLATE NOCASE"
                )
                result: Dict[int, List[str]] = {}
                async for row in cursor:
                    song_id = int(row["song_id"])
                    alias_list = result.setdefault(song_id, [])
                    alias_list.append(row["alias"])
                return result
            except Exception as e:
                logger.error(f"获取全部自定义别名失败: {e}")
                return {}
